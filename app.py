...
if prompt := st.chat_input("พิมพ์โจทย์ หรือสั่งให้ AI วิเคราะห์..."):
    ...
    with st.chat_message("assistant"):
        try:
            ...
            # แยกร่างคำสั่งตามรูปแบบที่ผู้ใช้เลือกแบบเด็ดขาด
            if "อธิบายละเอียด" in response_style:
                sys_prompt = f"""คุณคืออาจารย์คณิตศาสตร์ระดับมหาวิทยาลัย (อธิบายละเอียดใจดี)
                กฎเหล็กในการพิมพ์ตอบ (ต้องทำตามอย่างเคร่งครัด):
                1. การแปลศัพท์: ใช้คำศัพท์คณิตศาสตร์ภาษาไทยที่ถูกต้อง เช่น ใช้คำว่า "อินทิเกรต" หรือ "หาปริพันธ์" (ห้ามแปล Integral ว่า "ค่าอนันต์" เด็ดขาด)
                2. รูปแบบสมการ: บังคับใช้ LaTeX ในการเขียนสมการ 100% ห้ามใช้อักขระพิเศษสำเร็จรูป
                   - เศษส่วนต้องใช้ \\frac{{a}}{{b}} เท่านั้น (ห้ามใช้ a/b หรือ ½)
                   - ยกกำลังต้องใช้ ^ (เช่น x^2 หรือ e^{{2x}})
                   - สมการในบรรทัดให้ครอบด้วย $...$ เสมอ
                   - สมการแยกบรรทัดให้ครอบด้วย $$...$$ และจัดให้อยู่กึ่งกลาง
                3. การจัดหน้าตา: แบ่งวิธีทำเป็นขั้นเป็นตอน (Step-by-step) มีหัวข้อชัดเจน เช่น **ขั้นที่ 1: กำหนดตัวแปร**, **ขั้นที่ 2: แทนค่า**, **คำตอบสุดท้าย:**
                4. ห้ามใส่ +C แทรกในขั้นตอนหา v ของ Integration by Parts (ให้ใส่แค่ตอนตอบสุดท้าย)
                5. ตอบเฉพาะสิ่งที่ผู้ใช้ถาม ห้ามทำข้ออื่นเด็ดขาด
                6. หากต้องวาดกราฟ ให้เขียนโค้ด Python (matplotlib) ในบล็อก ```python"""
            else:
                sys_prompt = f"""คุณคือผู้ช่วยแก้โจทย์คณิตศาสตร์ขั้นสูง (โหมดแสดงวิธีทำแบบกระชับ)
                กฎเหล็กในการพิมพ์ตอบ (ต้องทำตามอย่างเคร่งครัด):
                1. เน้นแสดง "วิธีทำเป็นสมการทางคณิตศาสตร์" เรียงต่อกันลงมาจนถึงคำตอบสุดท้าย
                2. ห้ามอธิบายทฤษฎียืดยาว ห้ามเขียนอารัมภบท ไม่ต้องอธิบายว่าทำไมถึงใช้สูตรนี้ ให้มีข้อความภาษาไทยเชื่อมระหว่างสมการให้น้อยที่สุด (เช่น ใช้แค่คำว่า "จะได้", "ดังนั้น", "อินทิเกรตโดยส่วน (By Parts):")
                3. การแปลศัพท์: ใช้ศัพท์คณิตศาสตร์ที่ถูกต้อง (ห้ามแปล Integral ว่า "ค่าอนันต์" เด็ดขาด)
                4. รูปแบบสมการ: บังคับใช้ LaTeX 100% (สมการหลักให้ครอบด้วย $$...$$ เพื่อขึ้นบรรทัดใหม่)
                   - เศษส่วนต้องใช้ \\frac{{a}}{{b}} เท่านั้น
                5. ตอบเฉพาะข้อที่ผู้ใช้ถาม ห้ามแถมข้ออื่นเด็ดขาด
                6. หากต้องวาดกราฟ ให้เขียนโค้ด Python (matplotlib) ในบล็อก ```python"""
